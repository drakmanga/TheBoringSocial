...
try {
    ...
    if (isset($_POST["removePost"])) {
        ...
        $allUserFollow = $followService->getAllMyFollow($user->getId());
        
        foreach ($allUserFollow as $follow) {
            $followId[] = $follow->getFollow_user_id();
        }
    ...
    foreach ($allPostFollow as $post) {
       
        
        if ($post->getUser_id() == $user->getId()) {
            $myPost = sprintf('<!-- bottone rimuovi post -->
            <div>
            ...
            </div>', $post->getId(), $post->getId(), $post->getId(),$post->getId(), $post->getId(), $post->getDescription(), $post->getId(),);
        }else {
            $myPost = "";
        }

        $userPostData = $userService->catchUserDataWithId($post->getUser_id());
        ...
        foreach($commentsPost as $commentPost) {
            ...
            <div class="container">
                <div class="row">
                    <div class="col col-lg-1">
                        <a class="pull-left" href="../php/profile.php?username=%s">
                            <div class="user"><img src="%s" ></div>
                        </a>
                    </div>
                    <div class="col-sm">
                        <a href="../php/profile.php?username=%s">
                            <p >%s</hp>
                        </a>
                    </div>
                    <div class="col-sm">
            ...
            $userData->getUsername(), $userData->getImagePath(), $userData->getUsername(), $userData->getName() . $userData->getSurname(), $commentPost->getDate(), $commentPost->getComment());
        }
        ...
        <div class="timeline-body">
            <div class="timeline-header">
                <form method="post" action="../php/dashboard.php" enctype="multipart/form-data">
        ...
        <li>',  $post->getDate(), $userPostData->getImagePath(), $userPostData->getName() . " ". $userPostData->getSurname(),
                $update, $datePublicateOrUpdate, $myPost, $post->getDescription(), $file ,$post->getId(),$post->getId(), 
                count($allCommentPost), count($likeCount), $post->getId(), $like, $post->getId(), $post->getId(), 
